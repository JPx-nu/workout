...
export async function normalizeAndStore(
    ...
    for (const w of workouts) {
        try {
            const windowStart = new Date(
                w.startedAt.getTime() - DEDUP_WINDOW_MS,
            ).toISOString();
            ...
            if (error) {
                console.error("[Normalizer] Workout insert error:", error);
                result.workoutsSkipped++;
            } else {
                result.workoutsInserted++;
            }
        } catch (err) {
            console.error("[Normalizer] Workout processing error:", err);
            result.workoutsSkipped++;
        }
    }
    ...
    for (const m of metrics) {
        try {
            ...
            if (error) {
                console.error("[Normalizer] Metric insert error:", error);
                result.metricsSkipped++;
            } else {
                result.metricsInserted++;
            }
        } catch (err) {
            console.error("[Normalizer] Metric processing error:", err);
            result.metricsSkipped++;
        }
    }
...
async function autoPopulateDailyLogs(
    ...
    const byDate = new Map<
        string,
        { hrv?: number; restingHr?: number; sleepHours?: number }
    >();
    ...
    for (const [date, data] of byDate) {
        ...
        if (existing) {
            ...
            if (existing.hrv === null && data.hrv !== undefined)
                updates.hrv = data.hrv;
