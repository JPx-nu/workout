...
import type { IntegrationProvider, ConnectedAccount } from "./types.js";
...
export async function handleOAuthCallback(
    ...
    if (upsertError) {
        console.error(
            `[OAuth] Failed to store tokens for ${provider.name}:`,
            upsertError,
        );
        throw new IntegrationError(
            provider.name,
            `Failed to store connection: ${upsertError.message}`,
        );
    }

    console.log(
        `[OAuth] ${provider.name} connected for athlete ${athleteId} (provider UID: ${tokens.providerUserId})`,
    );
    ...
    backfillActivities(
        ...
        .then((result) => {
            console.log(
                `[OAuth] ${provider.name} backfill complete: ${result.workoutsInserted} workouts, ${result.metricsInserted} metrics`,
            );
        })
        .catch((err) => {
            console.error(`[OAuth] ${provider.name} backfill failed:`, err);
        });
...
async function backfillActivities(
    ...
    if (provider.fetchHealthData) {
        const todayMetrics = await provider.fetchHealthData(
            accessToken,
            new Date(),
        );
...
export async function disconnectProvider(
    ...
    if (account) {
        ...
        try {
            ...
            console.warn(
                `[OAuth] Failed to revoke ${provider.name} access (continuing):`,
                err,
            );
        }
    ...
    console.log(
        `[OAuth] ${provider.name} disconnected for athlete ${athleteId}`,
    );
}
