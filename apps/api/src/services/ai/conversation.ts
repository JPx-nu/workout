...
export async function saveMessages(
    client: SupabaseClient,
    conversationId: string,
    messages: Array<{ role: 'user' | 'assistant' | 'system'; content: string; metadata?: Record<string, unknown> }>
): Promise<void> {
...
export async function listConversations(
    ...
    userId: string,
    limit = 20
): Promise<Array<{ id: string; title: string | null; created_at: string; message_count: number }>> {
    const { data, error } = await client
        .from('conversations')
