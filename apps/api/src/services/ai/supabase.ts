...
export async function getWorkouts(
    client: SupabaseClient,
    userId: string,
    options: { limit?: number; fromDate?: string; toDate?: string; activityType?: string } = {}
): Promise<Workout[]> {
    let query = client
        .from('workouts')
        .select('*')
...
export async function getUpcomingEvents(
    ...
    userId: string,
    limit = 5
): Promise<Array<{ id: string; name: string; event_date: string; distance_type: string | null }>> {
...
export async function getInjuries(
    ...
    userId: string,
    activeOnly = true
): Promise<Array<{ id: string; body_part: string; severity: number | null; reported_at: string; resolved_at: string | null; notes: string | null }>> {
    let query = client
        .from('injuries')
