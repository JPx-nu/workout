...
export function createSearchWorkoutsTool(client: SupabaseClient, userId: string) {
    return tool(
        async ({ query, threshold = 0.6, limit = 5 }) => {
            ...
            return JSON.stringify(data.map((w: any) => ({
                id: w.id,
        ...
        },
        {
            ...
            schema: z.object({
                query: z.string().describe('The natural language search query (e.g. "rainy run", "felt great")'),
                threshold: z.number().optional().describe('Minimum similarity threshold (0-1). Default 0.6'),
