...
export function createGetHealthMetricsTool(client: SupabaseClient, userId: string) {
    return tool(
        async ({ days, metricType }: Record<string, string | number | undefined>) => {
            ...
            const [dailyLogs, healthMetrics] = await Promise.all([
                ...
                getHealthMetrics(client, userId, { fromDate, metricType: metricType as string | undefined }),
            ]);
        ...
        },
        {
            ...
            schema: z.object({
                ...
                metricType: z.string().optional().describe('Filter health metrics by type (e.g., VO2_MAX, BODY_FAT)'),
            }) as any,
        }
    );
}
