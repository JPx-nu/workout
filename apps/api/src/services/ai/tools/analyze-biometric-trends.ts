    ...
    return tool(
        async ({ days = 30 }) => {
            ...
            const calculateAvgs = (logSet: typeof logs) => {
                ...
                logSet.forEach(l => {
                    if (l.sleep_hours != null) { s += l.sleep_hours; sc++; }
                    if (l.sleep_quality != null) { sq += l.sleep_quality; sqc++; }
                    if (l.hrv != null) { h += l.hrv; hc++; }
                    if (l.resting_hr != null) { rh += l.resting_hr; rhc++; }
                    if (l.rpe != null) { rp += l.rpe; rpc++; }
                });
        ...
        },
        {
            ...
            schema: z.object({
                days: z.number().min(7).max(90).optional().describe('Number of days to look back for the trend analysis (default 30)'),
            }),
        }
    );
}
