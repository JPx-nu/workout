...
export function createPassBatonTool(client: SupabaseClient, userId: string) {
    return tool(
        ...
        },
        {
            ...
            schema: z.object({
                toAthleteId: z.string().describe('The ID of the squad member to pass the baton to. You can get this ID from the get_squad_leaderboard tool.'),
                distanceMeters: z.number().describe('The distance in meters contributed to the relay leg by this pass.'),
                notes: z.string().optional().describe('Optional message or cheer to send to the next athlete.'),
            }) as any,
        }
    );
}
