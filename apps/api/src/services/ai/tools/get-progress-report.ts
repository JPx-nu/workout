...
export function createGetProgressReportTool(
	client: SupabaseClient,
	userId: string,
) {
	return tool(
		async ({ days }: Record<string, number | undefined>) => {
			const lookbackDays = (days as number | undefined) ?? 14;
			...
			const byActivity: Record<
				string,
				{ count: number; totalMin: number; totalKm: number }
			> = {};
			for (const w of workouts) {
				const key = w.activity_type;
				if (!byActivity[key])
					byActivity[key] = { count: 0, totalMin: 0, totalKm: 0 };
				byActivity[key].count++;
				byActivity[key].totalMin += w.duration_s
					? Math.round(w.duration_s / 60)
					: 0;
				byActivity[key].totalKm += w.distance_m
					? +(w.distance_m / 1000).toFixed(2)
					: 0;
			}

			// ── Health trends ──────────────────────────────────
			const sleepValues = dailyLogs
				.map((d) => d.sleep_hours)
				.filter((v): v is number => v != null);
			const hrvValues = dailyLogs
				.map((d) => d.hrv)
				.filter((v): v is number => v != null);
			const moodValues = dailyLogs
				.map((d) => d.mood)
				.filter((v): v is number => v != null);
			const rpeValues = dailyLogs
				.map((d) => d.rpe)
				.filter((v): v is number => v != null);
