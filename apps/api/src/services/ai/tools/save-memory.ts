...
export function createSaveMemoryTool(client: SupabaseClient, userId: string) {
    return tool(
        ...
        },
        {
            ...
            schema: z.object({
                category: z.enum(['preference', 'goal', 'constraint', 'pattern', 'medical_note', 'other']).describe('The category of the memory'),
                content: z.string().describe('The standalone fact to remember (e.g. "Athlete prefers evening workouts")'),
                importance: z.number().min(1).max(5).optional().describe('Importance of the memory from 1 (trivial) to 5 (critical)'),
            }),
        }
    );
}
