...
wahooRoutes.get("/callback", async (c) => {
    ...
    if (error || !code || !state) {
        ...
        return c.redirect(
            `${webUrl}/workout/settings?integration=wahoo&error=denied`,
        );
    }
    ...
    try {
        ...
        const webUrl = process.env.WEB_URL || "http://localhost:3000";
        return c.redirect(
            `${webUrl}/workout/settings?integration=wahoo&status=connected`,
        );
    } catch (err) {
        console.error("[Wahoo] OAuth callback failed:", err);
        const webUrl = process.env.WEB_URL || "http://localhost:3000";
        return c.redirect(
            `${webUrl}/workout/settings?integration=wahoo&error=failed`,
        );
    }
});
