...
stravaRoutes.get("/callback", async (c) => {
    ...
    if (error || !code || !state) {
        ...
        return c.redirect(
            `${webUrl}/workout/settings?integration=strava&error=denied`,
        );
    }
    ...
    try {
        ...
        const webUrl = process.env.WEB_URL || "http://localhost:3000";
        return c.redirect(
            `${webUrl}/workout/settings?integration=strava&status=connected`,
        );
    } catch (err) {
        console.error("[Strava] OAuth callback failed:", err);
        const webUrl = process.env.WEB_URL || "http://localhost:3000";
        return c.redirect(
            `${webUrl}/workout/settings?integration=strava&error=failed`,
        );
    }
});
