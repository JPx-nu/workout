...
polarRoutes.get("/callback", async (c) => {
    ...
    if (error || !code || !state) {
        ...
        return c.redirect(
            `${webUrl}/workout/settings?integration=polar&error=denied`,
        );
    }
    ...
    try {
        ...
        const webUrl = process.env.WEB_URL || "http://localhost:3000";
        return c.redirect(
            `${webUrl}/workout/settings?integration=polar&status=connected`,
        );
    } catch (err) {
        console.error("[Polar] OAuth callback failed:", err);
        const webUrl = process.env.WEB_URL || "http://localhost:3000";
        return c.redirect(
            `${webUrl}/workout/settings?integration=polar&error=failed`,
        );
    }
});
