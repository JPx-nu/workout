# Web App (`apps/web`)

Next.js 16 frontend for the current athlete-first product surface.

## Stack

- Next.js 16 App Router
- React 19
- Tailwind CSS v4
- `next-themes`
- Supabase auth/data clients
- Vercel AI SDK 6 (`@ai-sdk/react`)
- Serwist service worker / PWA manifest

## Local development

From repo root:

```bash
pnpm --filter web dev
```

Default port: `3100`.

Local guidance:
- Follow `apps/web/AGENTS.md` together with the root `AGENTS.md`.
- `apps/web/docs/MOCK_DATA_MIGRATION.md` is the current data-layer note for this workspace.

## Required env vars

- `NEXT_PUBLIC_SUPABASE_URL`
- `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY`
- `NEXT_PUBLIC_API_URL`
- `NEXT_PUBLIC_WEB_URL`

Important implementation note:
- In `apps/web/src`, read public env values with static access like `process.env.NEXT_PUBLIC_SUPABASE_URL`.
- Do not use dynamic access like `process.env[name]` in browser/shared modules. Next standalone may not inline those values into client bundles, which can break production even when Azure app settings are correct.

Optional feature flags:

- `NEXT_PUBLIC_ENABLE_GOOGLE_AUTH`
- `NEXT_PUBLIC_ENABLE_DEMO`

## Current routes and behavior

- `basePath` is `/workout` (see `next.config.ts`).
- `/health` is the deployment health endpoint and resolves to `/workout/health` in production because of the base path.
- `/` is the current marketing/landing page.
- `/login` handles Supabase auth entry.
- `/dashboard/*` holds the shipped app surface.
- `/dashboard/workouts/new` is the Workout Center for `start now`, `log past`, and `schedule`.
- `/dashboard/body-map-3d` still exists but remains experimental and uses sample fatigue data.
- AI Coach streams through `/api/ai/stream`.
- Manual completed workout writes use `/api/workouts`.
- Planned workouts use `/api/planned-workouts`.
- Multi-session scheduling uses `/api/planned-workouts/batch`.
- Integrations settings use `/api/integrations/status` plus provider action routes.
- Service worker output is generated by Serwist and disabled in development.
- In deployed environments, `NEXT_PUBLIC_API_URL` must point at the real API origin. Web CSP `connect-src` is built from that value at build time.
- Azure App Service should run this app on Node `24.x`, keep `alwaysOn` enabled, and probe `/workout/health` for Health Check.
- The GitHub deploy workflow smoke-tests `/workout/` after rollout and then exercises the published `/api/ai/stream` path, so production-only env/config issues should fail the deploy instead of silently shipping.

## Build

```bash
pnpm --filter web build
pnpm --filter web start
```

## Live Browser Tests

Playwright browser tests live in `apps/web/e2e` and are intended to run against a real environment with live data, not mocks.

Required env vars:

- `PLAYWRIGHT_BASE_URL` - optional override for the full app base URL including `/workout`
- `PLAYWRIGHT_TEST_EMAIL` - optional athlete test account email
- `PLAYWRIGHT_TEST_PASSWORD` - optional athlete test account password

Local default behavior:

- if `PLAYWRIGHT_BASE_URL` is unset, Playwright targets `http://localhost:3100/workout`
- local runs auto-start the real API and web dev servers through repo scripts
- if `PLAYWRIGHT_TEST_EMAIL` and `PLAYWRIGHT_TEST_PASSWORD` are both unset, auth falls back to the live demo athlete account
- if you set one credential env var, you must set both

Install browser binaries:

```bash
pnpm --filter web playwright:install
```

Run the suite:

```bash
pnpm --filter web test:e2e
```

PR smoke slice:

```bash
pnpm --filter web test:e2e:smoke
```

Broader desktop + mobile-web matrix:

```bash
pnpm --filter web test:e2e:matrix
```

Current browser coverage focuses on:

- live login through Supabase auth
- desktop + mobile-web route smoke coverage for login, dashboard, coach, training, workouts, Workout Center, and settings
- opening the Workout Center from workout history
- logging a past strength workout
- scheduling a future strength session
- sending an AI Coach quick-log flow and verifying the workout appears in history

Accessibility and performance:

- `@axe-core/playwright` runs in the smoke suite and attaches violations to Playwright artifacts
- set `PLAYWRIGHT_STRICT_A11Y=true` if you want local runs to fail on serious/critical axe findings
- `pnpm --filter web test:lighthouse` runs Lighthouse CI against the critical routes

Visual review:

- `pnpm --filter web storybook` starts local Storybook
- `pnpm --filter web storybook:build` builds the Storybook/Chromatic bundle
- the first visual stories live alongside the app in `src/**/*.stories.tsx`

Artifact output:

- generated QA artifacts are written to repo-level `.qa-artifacts/`
