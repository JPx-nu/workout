...
import type { MuscleFatigue, DailyLog, HealthSnapshot, FatigueLevel } from '@/lib/mock/health';
...
export function useHealth() {
    ...
    const [healthSnapshot, setHealthSnapshot] = useState<HealthSnapshot>(defaultSnapshot);
        ...
        if (logData && logData.length > 0) {
            ...
            setHealthSnapshot({
                ...
                readinessScore: Math.min(100, Math.round(
                    (latest.sleepQuality * 10 + latest.mood * 5 + (latest.hrv > 0 ? 30 : 0)) / 1.4,
                )),
            });
        } else {
            setDailyLogs([]);
