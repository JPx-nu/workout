...
import { Dumbbell, Activity, Timer, TrendingUp, ChevronRight } from 'lucide-react';
...
export function StrengthView({ workouts, metrics }: StrengthViewProps) {
    ...
    const chartData = [1, 2, 3, 4, 5, 6, 0].map((offset) => {
        ...
        const volume = dayWorkouts.reduce((acc, w) => {
            ...
            if (!data) return acc;
            const sessionVol = data.exercises.reduce((sAcc, ex) =>
                sAcc + ex.sets.reduce((setAcc, s) => setAcc + (s.weightKg * s.reps), 0), 0);
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 lg:gap-4 stagger-children">
                {stats.map((stat) => (
                    <SpotlightCard key={stat.label} className="p-4 lg:p-5">
                        ...
                        {stat.sub && (
                            <div className="text-xs mt-1" style={{ color: 'var(--color-text-muted)' }}>{stat.sub}</div>
                        )}
                    </SpotlightCard>
                ))}
            </div>
                            ...
                            <BarChart data={chartData} barGap={2}>
                                ...
                                <Bar dataKey="volume" radius={[4, 4, 0, 0]}>
                                    {chartData.map((entry, index) => (
                                        <Cell key={`cell-${index}`} fill="var(--color-strength)" opacity={entry.volume > 0 ? 0.8 : 0.1} />
                                    ))}
                                </Bar>
                            </BarChart>
                        </ResponsiveContainer>
            ...
            <div className="glass-card p-4 lg:p-6">
                ...
                <div className="space-y-3">
                    {recentWorkouts.map((w) => {
                        const data = w.rawData as StrengthSessionData | undefined;
                        const volume = data?.exercises.reduce((act, ex) =>
                            act + ex.sets.reduce((sact, s) => sact + (s.weightKg * s.reps), 0), 0) || 0;
                        ...
                        return (
                            <div key={w.id} className="flex items-center gap-4 p-3 rounded-xl transition-colors hover-surface">
                                <div className="w-10 h-10 rounded-lg flex items-center justify-center shrink-0"
                                    style={{ background: 'color-mix(in oklch, var(--color-strength), transparent 85%)' }}>
