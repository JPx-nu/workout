...
import {
    ...
    X, Clock, MapPin, Flame, Pencil, Trash2,
} from 'lucide-react';
import { usePlannedWorkouts, type PlannedWorkout } from '@/hooks/use-planned-workouts';
...
const ACTIVITY_CONFIG: Record<string, {
    ...
    SWIM: {
        icon: Waves,
        color: '#06b6d4',
        lightBg: '#cffafe', darkBg: '#155e75',
        lightText: '#0e7490', darkText: '#a5f3fc',
    },
    BIKE: {
        icon: Bike,
        color: '#f59e0b',
        lightBg: '#fef3c7', darkBg: '#78350f',
        lightText: '#b45309', darkText: '#fde68a',
    },
    RUN: {
        icon: Footprints,
        color: '#10b981',
        lightBg: '#d1fae5', darkBg: '#064e3b',
        lightText: '#047857', darkText: '#6ee7b7',
    },
    STRENGTH: {
        icon: Dumbbell,
        color: '#8b5cf6',
        lightBg: '#ede9fe', darkBg: '#4c1d95',
        lightText: '#6d28d9', darkText: '#c4b5fd',
    },
    YOGA: {
        icon: Activity,
        color: '#ec4899',
        lightBg: '#fce7f3', darkBg: '#831843',
        lightText: '#be185d', darkText: '#f9a8d4',
    },
    OTHER: {
        icon: Zap,
        color: '#6b7280',
        lightBg: '#f3f4f6', darkBg: '#374151',
        lightText: '#4b5563', darkText: '#d1d5db',
    },
};
    ...
    const calendars = useMemo(() => {
        ...
        for (const [key, cfg] of Object.entries(ACTIVITY_CONFIG)) {
            cal[key.toLowerCase()] = {
                ...
                lightColors: { main: cfg.color, container: cfg.lightBg, onContainer: cfg.lightText },
                darkColors: { main: cfg.color, container: cfg.darkBg, onContainer: cfg.darkText },
            };
        }
        return cal;
    }, []);
...
function WorkoutDetailModal({
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
            ...
            <div className="glass-card rounded-2xl w-full max-w-md p-6 animate-scale-in"
                onClick={(e) => e.stopPropagation()}>
                {/* Header */}
                <div className="flex items-start justify-between mb-4">
                    <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-xl flex items-center justify-center"
                            style={{ background: `color-mix(in oklch, ${cfg.color}, transparent 80%)` }}>
                ...
                <div className="space-y-3 mb-5">
                    ...
                    {workout.description && (
                        <div className="mt-3 p-3 rounded-xl text-sm"
                            style={{ background: 'var(--color-glass-bg-subtle)', color: 'var(--color-text-secondary)' }}>
                            {workout.description}
                        </div>
                    )}
                ...
                <div className="flex items-center gap-2 mb-4">
                    <span className="text-xs font-medium px-2.5 py-1 rounded-full"
                        style={{ color: statusInfo.color, background: `color-mix(in oklch, ${statusInfo.color}, transparent 85%)` }}>
                        {statusInfo.label}
                    </span>
                    {workout.source === 'AI' && (
                        <span className="text-xs font-medium px-2.5 py-1 rounded-full"
                            style={{ color: 'var(--color-brand)', background: 'var(--color-glass-bg-subtle)' }}>
                            AI Generated
                        </span>
                    )}
                </div>
