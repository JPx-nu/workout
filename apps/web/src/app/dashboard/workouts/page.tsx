...
import { Waves, Bike, Footprints, Dumbbell, Filter } from 'lucide-react';
import { useWorkouts, formatDuration, mToKm, formatPace } from '@/hooks/use-workouts';
...
const badgeClasses: Record<string, string> = {
    SWIM: 'badge-swim', BIKE: 'badge-bike', RUN: 'badge-run', STRENGTH: 'badge-strength',
};
                                ...
                                <div className="hidden sm:flex gap-6 shrink-0 text-right">
                                    ...
                                    {w.distanceM && (
                                        <div>
                                            ...
                                            <div className="text-xs" style={{ color: 'var(--color-text-muted)' }}>Distance</div>
                                        </div>
                                    )}
                                    {w.avgHr && (
                                        <div>
                                            <div className="text-sm font-semibold">{w.avgHr} bpm</div>
                                            <div className="text-xs" style={{ color: 'var(--color-text-muted)' }}>Avg HR</div>
                                        </div>
                                    )}
                                    {w.tss && (
                                        <div>
                                            <div className="text-sm font-semibold">{w.tss}</div>
                                            <div className="text-xs" style={{ color: 'var(--color-text-muted)' }}>TSS</div>
                                        </div>
                                    )}
                                    {w.distanceM && w.activityType === 'RUN' && (
                                        <div>
                                            <div className="text-sm font-semibold">{formatPace(w.durationSec, w.distanceM)}</div>
                                            <div className="text-xs" style={{ color: 'var(--color-text-muted)' }}>Pace</div>
                                        </div>
                                    )}
                                </div>
                            </div>
