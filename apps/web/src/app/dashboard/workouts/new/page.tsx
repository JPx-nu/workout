...
function parseMode(value: string | null): WorkoutCenterMode {
	...
	if (value === "past" || value === "log_past") return "log_past";
	if (value === "schedule") return "schedule";
	return "log_past";
}

function parseActivity(value: string | null): ActivityType {
	return ACTIVITY_OPTIONS.includes(value as ActivityType) ? (value as ActivityType) : "STRENGTH";
}
...
export default function NewWorkoutPage() {
	...
	const initialActivity = parseActivity(searchParams.get("activity"));
	const requestedPlannedId = searchParams.get("plannedWorkoutId");
	const requestedWorkoutId = searchParams.get("workoutId");
	const wideRange = useMemo(() => getWideRange(), []);

	const { allWorkouts, isLoading: workoutsLoading, refetch: refetchWorkouts } = useWorkouts();
	const {
		workouts: draftSessions,
		isLoading: draftsLoading,
		refetch: refetchDrafts,
	} = usePlannedWorkouts(wideRange.from, wideRange.to, "in_progress");
	const {
		workouts: plannedSessions,
		isLoading: plannedLoading,
		refetch: refetchPlanned,
	} = usePlannedWorkouts(wideRange.from, wideRange.to);
	const {
