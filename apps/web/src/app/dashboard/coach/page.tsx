...
import { Send, Bot, User, Sparkles, History, ChevronLeft, ChevronRight, MessageSquare, Paperclip, X, Image as ImageIcon } from 'lucide-react';
import { useCoach } from '@/hooks/use-coach';
import { Plus } from 'lucide-react';

export default function CoachPage() {
    const {
        messages, isTyping, input, setInput, sendMessage, suggestedPrompts,
        conversations, loadConversation, newConversation, conversationId, error, activeToolCalls,
                    ...
                    <div className="flex gap-2 pt-2 max-w-4xl mx-auto w-full shrink-0">
                        {attachedFiles.map((file, i) => (
                            <div key={i} className="relative group">
                                ...
                                <div className="absolute bottom-0 left-0 right-0 text-[8px] text-center truncate px-0.5 py-0.5 rounded-b-lg"
                                ...
                                <div className="flex items-start gap-2.5">
                                    ...
                                    <div className="min-w-0 flex-1">
                                        <div
                                            ...
                                            style={{ color: isActive ? 'var(--color-brand-light)' : 'var(--color-text-secondary)' }}
                                        >
                                            {conv.title || 'Untitled'}
                                        </div>
                                        <div className="flex items-center gap-1.5 mt-1" style={{ color: 'var(--color-text-muted)' }}>
                                            ...
                                            <span>·</span>
                                            <span>{new Date(conv.updated_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}</span>
                                        </div>
                                    </div>
                                </div>
                            </button>
