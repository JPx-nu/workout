...
export default function CoachPage({
	...
	initialState?: Partial<CoachPageBootstrap>;
}) {
	const {
		messages,
		uiMessages,
				...
				>
					...
					<button
						...
						className={`btn-primary px-4 disabled:opacity-40 disabled:cursor-not-allowed ${
							isResponding && !input.trim() ? "animate-pulse" : ""
						}`}
						style={
						...
						aria-label={isResponding && !input.trim() ? "Stop generating" : "Send message"}
					>
						{isResponding && !input.trim() ? (
