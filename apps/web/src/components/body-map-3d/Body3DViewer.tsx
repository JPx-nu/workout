...
const MUSCLE_NAME_MAP: Record<string, string> = {
    ...
    'rectus abdominis': 'core', 'external oblique': 'core',
    'internal oblique': 'core', 'transversus abdominis': 'core',
    'latissimus dorsi': 'lats', 'trapezius': 'traps', 'rhomboid': 'lats',
    'erector spinae': 'lower_back',
    'infraspinatus': 'shoulders', 'supraspinatus': 'shoulders', 'teres': 'lats',
    'quadriceps': 'quadriceps', 'rectus femoris': 'quadriceps',
    ...
    'iliopsoas': 'hip_flexors', 'psoas': 'hip_flexors',
    'adductor': 'adductors', 'adductor longus': 'adductors',
    'adductor magnus': 'adductors', 'gracilis': 'adductors',
    'sternocleidomastoid': 'neck', 'scalene': 'neck',
};
...
const FATIGUE_COLORS = {
    ...
    moderate: { base: new THREE.Color(0xffb800), emissive: new THREE.Color(0xffb800) },
    high: { base: new THREE.Color(0xff3d5a), emissive: new THREE.Color(0xff3d5a) },
};
...
function BodyModel({
    ...
    const fatigueLookup = useMemo(() => {
        ...
        fatigueData.forEach(f => { map[f.bodyPart] = f.level; });
        return map;
    }, [fatigueData]);
    ...
    useEffect(() => {
        ...
        scene.traverse((child: THREE.Object3D) => {
            ...
            const userData = mesh.userData as { type?: string; name?: string; nameDetail?: string };
            ...
            if (userData.type === 'bone') {
                ...
                mesh.material = mat;
                muscleMeshes.push({ mesh, material: mat, baseEmissiveIntensity, baseOpacity });
            } else {
                mesh.material = new THREE.MeshPhysicalMaterial({
                    color: 0x151c28,
                    transparent: true,
    ...
    useFrame(({ clock }) => {
        ...
        const pulse = 0.35 + Math.sin(t * Math.PI) * 0.20;
        const opacityPulse = 0.85 + Math.sin(t * Math.PI) * 0.10;
    ...
    const handleClick = useCallback((e: any) => {
        e.stopPropagation();
        const mesh = e.object as THREE.Mesh;
        const ud = mesh.userData as { type?: string; name?: string; nameDetail?: string };
        if (ud.type === 'muscle') {
...
export default function Body3DViewer({ fatigueData }: { fatigueData: MuscleFatigue[] }) {
    const [selectedMuscle, setSelectedMuscle] = useState<{ name: string; bodyPart: string | null } | null>(null);
            ...
            <div className="flex-1 w-full">
                ...
                >
                    ...
                    >
                        ...
                        <pointLight position={[-3, 2, -2]} intensity={0.4} color="#6366f1" distance={10} decay={2} />
                        <pointLight position={[3, -1, -3]} intensity={0.3} color="#0ea5e9" distance={10} decay={2} />
                        {/* Subtle top fill */}
                        <pointLight position={[0, 6, 0]} intensity={0.15} color="#a78bfa" distance={12} decay={2} />
            ...
            <div className="w-full lg:w-80 lg:sticky lg:top-6">
                ...
                >
                            ...
                            <div className="flex items-center gap-5">
                                <div className="relative w-20 h-20 flex-shrink-0">
                                    ...
                                    <span className="absolute inset-0 flex items-center justify-center text-lg font-bold"
                                        style={{ color: getFatigueTheme(selectedFatigue.level).base.getStyle() }}>
                                        {selectedFatigue.level}%
                                    </span>
                                </div>
                                <div className="space-y-1">
                                    ...
                                    <div className="text-xs leading-relaxed" style={{ color: 'rgba(255,255,255,0.4)' }}>
                                        {selectedFatigue.status === 'high' ? 'Critical fatigue — rest recommended' :
                                            selectedFatigue.status === 'moderate' ? 'Moderate fatigue — limit intensity' :
                    ...
                    <div className="space-y-1.5">
                        ...
                        {[...fatigueData].sort((a, b) => b.level - a.level).map(f => {
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <span className="text-[11px] font-bold w-8 text-right" style={{ color: colorStr }}>
                                        {f.level}%
                                    </span>
                                </div>
                            );
